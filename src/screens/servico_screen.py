...
class ServicoScreen:
    def __init__(self, root):
        self.root = root
        self.root.title("Gestão de Serviços - SGS")
        ...
        tk.Label(root, text="ID Grupo:").grid(row=4, column=0)
        self.entry_id_grupo = tk.Entry(root)
        self.entry_id_grupo.grid(row=4, column=1)

        tk.Button(root, text="Cadastrar", command=self.cadastrar).grid(row=5, column=0)
        tk.Button(root, text="Buscar", command=self.buscar).grid(row=5, column=1)
        tk.Button(root, text="Atualizar", command=self.atualizar).grid(row=6, column=0)
        tk.Button(root, text="Excluir", command=self.excluir).grid(row=6, column=1)
        tk.Button(root, text="Limpar", command=self.limpar).grid(row=7, column=0, columnspan=2)
