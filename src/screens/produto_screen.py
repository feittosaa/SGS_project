...
class ProdutoScreen:
    def __init__(self, root):
        self.root = root
        self.root.title("Gestão de Produtos - SGS")
        ...
        tk.Label(root, text="Quantidade disponível:").grid(row=4, column=0)
        self.entry_quantidade = tk.Entry(root)
        self.entry_quantidade.grid(row=4, column=1)

        tk.Label(root, text="ID Grupo:").grid(row=5, column=0)
        self.entry_id_grupo = tk.Entry(root)
        self.entry_id_grupo.grid(row=5, column=1)

        tk.Button(root, text="Cadastrar", command=self.cadastrar).grid(row=6, column=0)
        tk.Button(root, text="Buscar", command=self.buscar).grid(row=6, column=1)
        tk.Button(root, text="Atualizar", command=self.atualizar).grid(row=7, column=0)
        tk.Button(root, text="Excluir", command=self.excluir).grid(row=7, column=1)
        tk.Button(root, text="Limpar", command=self.limpar).grid(row=8, column=0, columnspan=2)
    ...
    def get_data(self):
        return {
            ...
            "quantidade_disponivel": float(self.entry_quantidade.get()),
    ...
    def preencher(self, data):
        ...
        self.entry_categoria.insert(0, data.get("categoria", ""))
        self.entry_valor.delete(0, tk.END)
        self.entry_valor.insert(0, str(data.get("valor", "")))
        self.entry_quantidade.delete(0, tk.END)
        self.entry_quantidade.insert(0, str(data.get("quantidade_disponivel", "")))
